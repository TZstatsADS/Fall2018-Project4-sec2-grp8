...
## Regressor
```{r regressor}
load("../output/data_set.RData")
data_set$factory = factor(data_set$y)
set.seed(1)

## seperate train set and test set
## we split the data set to about 70% train set and 30% test set
num_candidate = 5 # number of candidate for each error
num_error = nrow(data_set)/num_candidate
samps = sample(1:10, num_error, replace = T)
all_error = 1:num_error
train_error = all_error[samps <= 7]
test_error = all_error[samps > 7]
indFunc = function(index){
  return((num_candidate*index - (num_candidate-1)):(index*num_candidate))
}
train_index = as.numeric(sapply(train_error, indFunc))
test_index = as.numeric(sapply(test_error, indFunc))
train_set = data_set[train_index, ]
test_set = data_set[test_index, ]

## train regressor
reg = adam2(factory~feature1+feature2+feature3+feature5+feature6, data = train_set, size = 10, alg = "c50")
...
test_outcome = data.frame(prob = test_prob, 
                          test_set$error_term, test_set$gt_term, test_set$candidate,
                          pair = rep(1:(nrow(test_outcome)/num_candidate), each = num_candidate)) %>%
  group_by(pair) %>%
